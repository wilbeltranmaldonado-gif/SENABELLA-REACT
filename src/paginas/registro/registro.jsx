...
function Registro() {
  ...
  let fortalezaEstilos = { width: "0%", bg: "transparent", texto: "Ingresa una contraseña", color: "var(--color-texto-secundario)" };
  if (formData.contrasena.length > 0) {
    if (fortaleza < 2) {
      fortalezaEstilos = { width: "33%", bg: "var(--color-error)", texto: "Contraseña Débil", color: "var(--color-error)" };
    } else if (fortaleza < 4) {
      fortalezaEstilos = { width: "66%", bg: "var(--color-advertencia)", texto: "Contraseña Media", color: "var(--color-advertencia)" };
    } else {
      fortalezaEstilos = { width: "100%", bg: "var(--color-exito)", texto: "Contraseña Fuerte", color: "var(--color-exito)" };
    }
  }
    ...
    <div className="auth-page">
      ...
      <main className="contenedor-registro">
        <div className="tarjeta-registro">
          ...
          <p className="subtitulo">Ingresa tus datos para comprar en Senabella</p>
          ...
          <form onSubmit={manejarSubmit} className="formulario" noValidate>
            ...
            <div className="grupo-campo">
              ...
              <div className="campo-wrapper">
                ...
                <button type="button" className="btn-toggle-pass" onClick={() => setMostrarContrasena(!mostrarContrasena)}>
              ...
              <div className="fortaleza-contenedor">
                <div className="fortaleza-barra">
                  <div className="fortaleza-progreso" style={{ width: fortalezaEstilos.width, backgroundColor: fortalezaEstilos.bg }}></div>
                </div>
                <span className="fortaleza-texto" style={{ color: fortalezaEstilos.color }}>{fortalezaEstilos.texto}</span>
              </div>
            </div>
            ...
            <div className="grupo-campo">
              ...
              <div className="campo-wrapper">
                ...
                <button type="button" className="btn-toggle-pass" onClick={() => setMostrarConfirmar(!mostrarConfirmar)}>
                  <i className={`fa-solid ${mostrarConfirmar ? 'fa-eye-slash' : 'fa-eye'}`}></i>
                </button>
              </div>
              {formData.confirmarContrasena.length > 0 && (
                <span className={`coincidencia-texto ${formData.contrasena === formData.confirmarContrasena ? 'valido' : 'invalido'}`}>
                  <i className={`fa-solid ${formData.contrasena === formData.confirmarContrasena ? 'fa-circle-check' : 'fa-circle-xmark'}`}></i>
            ...
            {mensajeError && (
              <div className="mensaje-error" style={{ display: 'flex' }}>
                <i className="fa-solid fa-triangle-exclamation"></i> <span>{mensajeError}</span>
              </div>
            )}
            {mensajeExito && (
              <div className="mensaje-exito" style={{ display: 'flex' }}>
                <i className="fa-solid fa-circle-check"></i> <span>{mensajeExito}</span>
              </div>
            )}
