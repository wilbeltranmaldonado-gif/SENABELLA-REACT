...
function Reportes() {
  ...
  const formatearMoneda = (val) =>
    "$ " + Math.round(val || 0).toLocaleString("es-CO");
  ...
  const ticketPromedio = useMemo(() => {
    return pedidosFiltrados.length > 0
      ? Math.round(totalIngresos / pedidosFiltrados.length)
      : 0;
  ...
  const clientesData = useMemo(() => {
    ...
    pedidosFiltrados.forEach((pedido) => {
      ...
      const ciudad =
        pedido.cliente?.ciudad || pedido.ciudad || "No especificada";
  ...
  const distribucionEstados = useMemo(() => {
    ...
    pedidosFiltrados.forEach((p) => {
      const estado =
        p.estado === "pendiente-verificacion"
          ? "pendiente"
          : p.estado || "pendiente";
  ...
  const desgloseMetodosPago = useMemo(() => {
    ...
    pedidosFiltrados.forEach((p) => {
      ...
      if (metodo.toLowerCase().includes("nequi")) metodo = "Nequi";
        ...
        metodo.toLowerCase().includes("contraentrega") ||
        metodo.toLowerCase().includes("entrega")
      )
  ...
  const desgloseCategorias = useMemo(() => {
    ...
    pedidosFiltrados.forEach((p) => {
      ...
      prods.forEach((prod) => {
        ...
        const subtotal =
          parsearPrecio(prod.precioText || prod.precio) *
  ...
  const promedioItemsPorPedido = useMemo(() => {
    ...
    const totalItems = pedidosFiltrados.reduce(
      (acc, p) => acc + (p.items || p.productos?.length || 1),
      0,
    );
  ...
  const generarReporte = () => {
    ...
    if (window.SenabellaToast) {
      window.SenabellaToast(
        "Reporte generado con datos actualizados",
  ...
  const descargarReporteCSV = () => {
    ...
    if (tipoReporte === "ventas") {
      ...
      filas = productosVendidosData.map((p) => [
        ...
        totalIngresos > 0
          ? ((p.ingresos / totalIngresos) * 100).toFixed(1) + "%"
          : "0%",
      ]);
      ...
      filas = Object.entries(distribucionEstados.estados).map(
        ([estado, cant]) => [
          estado.toUpperCase(),
          cant,
  ...
  return (
    <div className='vista-reportes'>
      ...
      {reporteGeneradoInfo && (
        ...
        >
          <span>
            <i className='fa-solid fa-circle-check'></i> Reporte de{" "}
            <strong>{reporteGeneradoInfo.tipo}</strong> actualizado el{" "}
            {reporteGeneradoInfo.fecha} ({reporteGeneradoInfo.totalRegistros}{" "}
            registros).
          </span>
          <button
            onClick={() => setReporteGeneradoInfo(null)}
            style={{
              background: "none",
              border: "none",
              color: "inherit",
      ...
      <div className='admin-grid-estadisticas'>
        ...
        <div className='admin-tarjeta-estadistica'>
          ...
          <div className='admin-tarjeta-contenido'>
            ...
            <p className='admin-tarjeta-cambio positivo'>
              <i className='fa-solid fa-check'></i> {pedidosCompletadosCount}{" "}
              completados
            </p>
          </div>
        </div>
        ...
        <div className='admin-tarjeta-estadistica'>
          ...
          <div className='admin-tarjeta-contenido'>
            ...
            <p className='admin-tarjeta-cambio positivo'>
              ...
              {productoEstrella
                ? productoEstrella.nombre.substring(0, 16) + "..."
                : "Sin ventas"}
            </p>
          </div>
        </div>
      ...
      <div className='admin-seccion'>
        ...
        >
          <div>
            <h3
              className='admin-seccion-subtitulo'
            ...
            {desgloseMetodosPago.length > 0 ? (
              ...
              >
                {desgloseMetodosPago.map((metodo, i) => {
                  const porcentaje =
                    totalIngresos > 0
                      ? ((metodo.monto / totalIngresos) * 100).toFixed(1)
                      : 0;
                  return (
                    <div
                      key={i}
          ...
          >
            ...
            >
              ...
              >
                ...
                >
                  {metricaMayorVenta
                    ? formatearMoneda(metricaMayorVenta.valorNum)
                    : "$ 0"}
                </strong>
                <small style={{ fontSize: "11px", color: "#94a3b8" }}>
                  {metricaMayorVenta
                    ? metricaMayorVenta.id || metricaMayorVenta.numero
                    : "-"}
                </small>
              </div>
              ...
              >
                ...
                >
                  {metricaMenorVenta
                    ? formatearMoneda(metricaMenorVenta.valorNum)
                    : "$ 0"}
                </strong>
                <small style={{ fontSize: "11px", color: "#94a3b8" }}>
                  {metricaMenorVenta
                    ? metricaMenorVenta.id || metricaMenorVenta.numero
                    : "-"}
                </small>
              </div>
            ...
            {desgloseCategorias.length > 0 && (
              <div
                style={{ borderTop: "1px dashed #e2e8f0", paddingTop: "10px" }}
              >
                <span
                  style={{
                    fontSize: "11.5px",
      ...
      <div className='admin-seccion'>
        ...
        >
          <h3 className='admin-seccion-subtitulo' style={{ margin: 0 }}>
            {tipoReporte === "ventas" &&
              `Detalle de Ventas (${pedidosFiltrados.length})`}
            {tipoReporte === "productos" &&
              `Desglose por Producto (${productosVendidosData.length})`}
            {tipoReporte === "pedidos" && "Resumen por Estado de Pedido"}
            {tipoReporte === "clientes" &&
              `Cartera de Clientes (${clientesData.length})`}
          </h3>
        ...
        <div className='admin-tabla-contenedor'>
              ...
              <tbody>
                {pedidosFiltrados
                  .filter((p) => {
                    ...
                    return (
                      String(p.id || p.numero)
                        .toLowerCase()
                        .includes(q) ||
                      String(p.cliente?.nombre || p.cliente || "")
                        .toLowerCase()
                        .includes(q) ||
                      String(p.estado || "")
                        .toLowerCase()
                  ...
                  .map((pedido, idx) => (
                    <tr key={pedido.id || idx}>
                      ...
                      <td>
                        {pedido.items || pedido.productos?.length || 1} uds
                      </td>
                      <td>
              ...
              <tbody>
                {productosVendidosData
                  ...
                  .map((prod, idx) => {
                    const participacion =
                      totalIngresos > 0
                        ? ((prod.ingresos / totalIngresos) * 100).toFixed(1)
                        : 0;
                    return (
                      <tr key={idx}>
                        <td>
                        ...
                        <td>
                          <span className='admin-badge'>{prod.categoria}</span>
                        </td>
                        <td>{formatearMoneda(prod.precioUnitario)}</td>
                        <td>
                          <strong>{prod.cantidad}</strong> uds
                        </td>
                        <td style={{ color: "#2563eb", fontWeight: 700 }}>
                          {formatearMoneda(prod.ingresos)}
                        </td>
                        <td>
                          <div
                            style={{
          ...
          {tipoReporte === "pedidos" && (
            <table className='admin-tabla'>
              ...
              <tbody>
                {Object.entries(distribucionEstados.estados).map(
                  ([estado, cant]) => {
                    const totalPeds = pedidosFiltrados.length || 1;
                    ...
                    return (
                      <tr key={estado}>
                        <td>
                          <span
                            className={`admin-badge ${obtenerClaseEstado(estado)}`}
                        ...
                        <td>
                          <strong>{cant}</strong> pedidos
                        </td>
                        <td>{porcentaje}%</td>
                        <td>
                          <strong>
                            {formatearMoneda(
                              distribucionEstados.valores[estado],
                            )}
                          </strong>
                        </td>
                      </tr>
                ...
                {clientesData
                  ...
                  .map((cliente, idx) => (
                    <tr key={idx}>
                      ...
                      <td>
                        <strong>{cliente.totalCompras}</strong> pedidos
                      </td>
