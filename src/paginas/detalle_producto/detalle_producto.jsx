...
function leerProductoSeleccionado() {
  try {
    return JSON.parse(
      localStorage.getItem(PRODUCTO_SELECCIONADO_KEY) || "null",
    );
  } catch {
    return null;
  }
...
function leerProductosAdmin() {
  try {
    const productos = JSON.parse(
      localStorage.getItem(PRODUCTOS_ADMIN_KEY) || "[]",
    );
...
function precioAnterior(producto) {
  ...
  return precio
    ? `$ ${Math.round(precio * 1.25).toLocaleString("es-CO")}`
    : "$ 0";
}
...
function normalizarProducto(producto) {
  ...
  return {
    ...
    descripcion:
      ...
      `${nombre}. Producto seleccionado de Senabella con compra segura y soporte especializado.`,
    imagen,
    precioActual: precio,
...
function obtenerEspecificaciones(producto) {
  ...
  if (texto.includes("impresora") || texto.includes("smart tank"))
    return [
      ...
      "Sistema de tintas continuas con gran rendimiento",
    ];
  if (
    ...
    return [
      ...
      "Pantalla Full HD de alta definición",
      "Almacenamiento SSD y memoria RAM de alto rendimiento",
    ];
  if (texto.includes("starlink"))
    return [
      "Internet satelital de alta velocidad y baja latencia",
      "Kit estándar V4 de fácil instalación",
      "Diseñado para soportar condiciones climáticas extremas",
    ];
  if (texto.includes("tablet"))
    return [
      "Pantalla táctil con excelente resolución",
      "Sistema de altavoces para contenido multimedia",
      "Diseño ligero y cómodo para llevar",
    ];
  if (texto.includes("disco") || texto.includes("toshiba"))
    return [
      "Capacidad de almacenamiento masivo de 2 TB",
      "Conexión USB 3.0 compatible con PC y Mac",
      "Estuche de protección resistente a impactos",
    ];
  if (
    ...
    return [
      "Diseño actual y materiales seleccionados",
      "Acabados cómodos para uso diario",
      "Compra segura con soporte Senabella",
    ];
  return [
...
function DetalleProducto() {
  ...
  const [producto, setProducto] = useState(() =>
    normalizarProducto(leerProductoSeleccionado() || productosIniciales[0]),
  );
  ...
  const esFavorito =
    favoritosVersion >= 0 &&
    window.SenabellaFavoritos.esFavorito(producto.nombre);
  ...
  const totalFormateado =
    "$ " + Math.round(totalCalculado).toLocaleString("es-CO");
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "senabella-producto-seleccionado",
      actualizarProducto,
    );
    return () =>
  ...
  useEffect(() => {
    const actualizarItems = () => {
      try {
        setCarritoItems(
          JSON.parse(localStorage.getItem("senabella_cart_db") || "[]"),
        );
      } catch {
        setCarritoItems([]);
      }
  ...
  const cantidadEnCarrito = itemEnCarrito
    ? parseInt(itemEnCarrito.cantidad, 10) || 0
    : 0;
    ...
    <div className='detalle-producto-page'>
      <main className='pagina-producto'>
        <div className='contenedor-volver'>
          <Link to={producto.origen || "/catalogo"} className='btn-volver'>
        ...
        <section className='tarjeta-producto'>
          <div className='imagen-producto'>
            <button
              className={`btn-favorito-detalle${esFavorito ? " activo" : ""}`}
            ...
            <div className='mini-miniaturas'>
              {[producto.imagen, producto.imagen, producto.imagen].map(
                (imagen, indice) => (
                  <img
                    key={`${imagen}-${indice}`}
                    src={imagen}
                    alt={`Vista ${indice + 1} de ${producto.titulo}`}
                    onError={manejarErrorImagen}
          ...
          <div className='info-producto'>
            ...
            <div className='beneficios'>
              ...
              <span>
                <i className='fa-solid fa-credit-card'></i> Pago con tarjeta
              </span>
              <span>
                <i className='fa-solid fa-shield-halved'></i> Compra segura
              </span>
            </div>
            <ul className='lista-especificaciones'>
              {obtenerEspecificaciones(producto).map((item) => (
                <li key={item}>{item}</li>
              ))}
            </ul>
              ...
              <div className='detalle-cantidad-fila'>
                ...
                <div className='detalle-contador-grupo'>
                  <button
                    ...
                    title={
                      cantidad <= 1 ? "Mínimo 1 unidad" : "Disminuir cantidad"
                    }
                  >
                  ...
                  <button
                    ...
                    disabled={
                      cantidad >= unidadesRestantes || botonAgregarBloqueado
                    }
                    title={
                      cantidad >= unidadesRestantes
            ...
            <div className='acciones-producto'>
              ...
              >
                <i
                  className={`fa-solid ${botonAgregarBloqueado ? "fa-lock" : "fa-cart-plus"}`}
                ></i>
                {stockDisponible <= 0
                  ? "Producto agotado"
                  : cantidadEnCarrito >= stockDisponible
              ...
              <Link
                to={producto.origen || "/catalogo"}
                className='btn-terciario-catalogo'
              >
                <i className='fa-solid fa-store'></i> Volver al catálogo
              </Link>
            </div>
          </div>
        </section>
        ...
        <section className='recomendaciones'>
          <div className='recomendaciones-header'>
            ...
            <p>Descubre productos recomendados de Senabella.</p>
          </div>
        ...
        <section className='info-grid'>
          <article className='info-box'>
            ...
            <p>
              Combina diseño, rendimiento y una experiencia de compra confiable
          ...
          <article className='info-box'>
            <h3>Compra segura</h3>
            <p>
              Procesamos tu pedido con opciones de pago y seguimiento desde tu
              cuenta.
            </p>
          ...
          <article className='info-box'>
            ...
            <p>
              Cuenta con asistencia y soporte Senabella para resolver tus dudas
