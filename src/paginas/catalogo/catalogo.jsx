...
function Catalogo() {
  ...
  const nombresTecnologia = new Set(
    productosIniciales.map((producto) => producto.nombre),
  );
  const productosTecnologiaAdmin = productosAdministrados.filter(
    (producto) =>
      !producto.origenCatalogo || nombresTecnologia.has(producto.nombre),
  );
  const productosDisponibles = Array.from(
    new Map(
      [...productosTecnologiaAdmin, ...productosIniciales].map((producto) => [
        producto.nombre,
        producto,
      ]),
    ).values(),
  ...
  const productosFiltrados = productosDisponibles.filter((prod) => {
    ...
    const cumpleMarca =
      ...
      marcasSeleccionadas.includes(prod.marca);

    // Categoria
    const cumpleCategoria =
      categoriasSeleccionadas.length === 0 ||
      categoriasSeleccionadas.some((catSel) => {
        const cs = catSel.toLowerCase();
        if (cs === "ofertas") return !!prod.descuento;
        return (
          prodCat.includes(cs) ||
          prodNombre.includes(cs) ||
          prodMarca.includes(cs)
        );
      });
    ...
    const cumplePrecio =
      ...
      preciosSeleccionados.some((rango) => {
        ...
        if (rango === "$500.000 - $1.000.000")
          return precio >= 500000 && precio <= 1000000;
        if (rango === "$1.000.000 - $2.000.000")
  ...
  const abrirDetalle = (prod) => {
    localStorage.setItem(
      ...
      JSON.stringify({
        ...
        descripcion:
          prod.descripcion ||
          `${prod.nombre}. Producto seleccionado de Senabella con compra segura y soporte especializado.`,
        imagen: prod.imagen,
        origen: "/catalogo",
      }),
    );
      ...
      <section className='categorias-circulares'>
        {categoriasCirculares.map((cat, idx) => (
          <div
            ...
            style={{
              cursor: "pointer",
              transform: categoriasSeleccionadas.includes(cat.categoria)
                ? "scale(1.08)"
                : "",
            }}
          >
            <div
              className='imagen-cat'
              style={{
                border: categoriasSeleccionadas.includes(cat.categoria)
                  ? "2px solid #84b814"
                  : "",
              }}
            >
          ...
          <div className='contenido-menu'>
            ...
            <div className='filtro'>
              ...
              >
                Tipo de Entrega
                <i
                  className={`fa-solid ${filtroEntregaAbierto ? "fa-chevron-up" : "fa-chevron-down"}`}
                ></i>
              </div>
            ...
            <div className='filtro'>
              <div
                className='filtro1'
                onClick={() =>
                  setFiltroCategoriaAbierto(!filtroCategoriaAbierto)
                }
                style={{ cursor: "pointer" }}
              >
                Categoría
                <i
                  className={`fa-solid ${filtroCategoriaAbierto ? "fa-chevron-up" : "fa-chevron-down"}`}
                ></i>
              </div>
            ...
            <div className='filtro'>
              ...
              >
                Marca
                <i
                  className={`fa-solid ${filtroMarcaAbierto ? "fa-chevron-up" : "fa-chevron-down"}`}
                ></i>
              </div>
              {filtroMarcaAbierto && (
            ...
            <div className='filtro'>
              ...
              >
                Precio
                <i
                  className={`fa-solid ${filtroPrecioAbierto ? "fa-chevron-up" : "fa-chevron-down"}`}
                ></i>
              </div>
              {filtroPrecioAbierto && (
                <div className='opciones-filtro-lateral'>
            ...
            <div className='botones' style={{ scrollBehavior: "smooth" }}>
              {marcasBotones.map((marca) => (
                <button
                  key={marca}
                  className={
                    marcasSeleccionadas.includes(marca) ? "sn-activo" : ""
                  }
                  style={
                    marcasSeleccionadas.includes(marca)
                      ? { background: "#84b814", color: "#fff" }
                      : {}
                  }
          ...
          <div className='recomendacion'>
            ...
            <div className='num-pagina'>
              ...
              <i
                ...
                style={{ cursor: "pointer" }}
                onClick={() =>
                  setPaginaActual((prev) => Math.min(paginasTotales, prev + 1))
                }
              ></i>
            </div>
          </div>
          ...
          <div className='tarjeta-producto'>
            {productosPaginados.map((prod) => (
              <div className='tar-producto' key={`${prod.id}-${prod.nombre}`}>
                {prod.promocion && (
                  <span className='promocion'>{prod.promocion}</span>
                )}
                ...
                <div className='etiqueta'>
                  ...
                  {prod.envioGratis && (
                    <span className='etiqueta-envio'>Envío gratis</span>
                  )}
                </div>
