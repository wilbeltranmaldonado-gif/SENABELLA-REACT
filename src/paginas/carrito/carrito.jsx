...
function Carrito() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("senabella_products_updated", ajustarCantidadesAStockAdmin);
    return () => {
      window.removeEventListener("storage", ajustarCantidadesAStockAdmin);
      window.removeEventListener("senabella_products_updated", ajustarCantidadesAStockAdmin);
    };
  }, []);
  ...
  const cargarSugerenciasCatalogo = (aleatorio = false) => {
    try {
      ...
      const productosAdminValidos = Array.isArray(guardadosAdmin) ? guardadosAdmin : [];
      ...
      const listaCatalogo = todosCatalogo.map((prod) => {
        ...
        const precioNum = typeof prod.precioNumero === "number" ? prod.precioNumero : parsearPrecio(prod.precio);
        const precioTexto = prod.precio || formatoMoneda(precioNum);
        const precioAntiguo = prod.precioSecundario1 || prod.precioSecundario || prod.precioAntiguo || formatoMoneda(Math.round(precioNum * 1.2));
      ...
      const disponibles = listaCatalogo.filter((prod) => prod.stock > 0 && prod.imagen);
  ...
  const agregarAlCarrito = (producto) => {
    ...
    const existente = itemsCarrito.find(item => item.id === producto.id || item.nombre === producto.nombre);
    if (existente) {
      if (existente.cantidad >= stockMaximo) {
        if (window.SenabellaToast) {
          window.SenabellaToast(`No puedes agregar más unidades. Stock disponible: ${stockMaximo} uds.`, "fa-circle-info", "advertencia");
        }
        return;
      }
    ...
    setItemsCarrito(itemsCarrito.map(item => {
      if (item.id === id) {
        ...
        if (delta > 0 && item.cantidad >= stockMaximo) {
          if (window.SenabellaToast) {
            window.SenabellaToast(`Has alcanzado el límite de inventario disponible (${stockMaximo} uds).`, "fa-circle-info", "advertencia");
          }
          return item;
        }
                    ...
                    <div key={item.id}>
                      <div className="fila-producto">
                        ...
                        <div className="detalles-producto">
                          ...
                          >
                            <i className="fa-solid fa-boxes-stacked"></i> {stockMaximo} disponibles en stock
                          </span>
                        </div>
                        ...
                        <div className="caja-acciones-producto">
                          ...
                          <div className="selector-cantidad">
                            <button 
                              ...
                              title={item.cantidad <= 1 ? "Mínimo 1 unidad" : "Disminuir cantidad"}
                            >
                            ...
                            <button 
                              ...
                              title={item.cantidad >= stockMaximo ? `Límite de stock alcanzado (${stockMaximo} uds)` : "Aumentar cantidad"}
                              style={item.cantidad >= stockMaximo ? { opacity: 0.4, cursor: "not-allowed" } : {}}
                            >
              ...
              <div className="tarjeta-sugerencia" key={prod.id}>
                ...
                >
                  ...
                  <span className="badge-stock-sugerencia">
                    <i className="fa-solid fa-check"></i> Stock: {stockDisponible} uds
                  </span>
                </div>
