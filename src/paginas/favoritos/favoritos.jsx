...
function Favoritos() {
  ...
  const mostrarToast = (mensaje, tipo = "exito") => {
    if (window.SenabellaToast) {
      window.SenabellaToast(
        mensaje,
        tipo === "exito" ? "fa-circle-check" : "fa-circle-info",
        tipo,
      );
    }
  };
  ...
  const favoritosOrdenados = [...favoritos].sort((a, b) => {
    if (ordenSeleccionado === "nombre-az")
      return (a.nombre || "").localeCompare(b.nombre || "");
    if (ordenSeleccionado === "nombre-za")
  ...
  const nombresEnFavoritos = new Set(
    favoritos.map((f) => f.nombre?.trim().toLowerCase()),
  );
      ...
      <main className='favoritos-main'>
        <section className='favoritos-seccion'>
          {favoritos.length === 0 ? (
            <div className='favoritos-vacio'>
              ...
              <h2 className='favoritos-vacio-titulo'>
              ...
              <div className='favoritos-barra'>
                <div className='favoritos-barra-izq'>
                  <span className='favoritos-contador'>
                    {favoritosOrdenados.length} producto
                    {favoritosOrdenados.length !== 1 ? "s" : ""}
                  </span>
                </div>
                <div className='favoritos-barra-der'>
                  <label
                    className='favoritos-orden-label'
                    htmlFor='orden-favoritos'
                  >
                    Ordenar por:
                  </label>
                  <select
                  ...
                  <button
                    className='favoritos-btn-limpiar'
                  ...
                  <div className='fav-tarjeta' key={prod.nombre + idx}>
                    ...
                    <a href='#'>
                      <img
                        ...
                        alt={prod.nombre}
                        className='fav-imagen'
                      />
                    </a>
                    <div className='fav-etiquetas'>
                      {prod.etiqueta && (
                        <span className='fav-etiqueta'>{prod.etiqueta}</span>
                      )}
                      {prod.envioGratis && (
                    ...
                    {prod.precioSecundario && (
                      <div className='fav-precio-secundario'>
                        {prod.precioSecundario}
                      </div>
                    )}
                    <button
        ...
        {sugerencias.length > 0 && (
          <section className='favoritos-sugerencias'>
            <h2 className='favoritos-sugerencias-titulo'>
              ...
              {sugerencias.map((prod) => (
                <div className='fav-sugerencia-tarjeta' key={prod.id}>
                  ...
                  <div className='fav-sugerencia-precio'>
                    ...
                    {prod.descuento && (
                      <span className='fav-sugerencia-descuento'>
                        {prod.descuento}
                      </span>
                    )}
                  </div>
                  <div className='fav-sugerencia-acciones'>
                    ...
                    <button
                      ...
                      onClick={() => {
                        iniciarFavoritosGlobal();
                        const agregado =
                          window.SenabellaFavoritos.toggleFavorito(prod);
                        mostrarToast(
                          agregado
